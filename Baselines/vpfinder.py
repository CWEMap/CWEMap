...
OUTPUT_DIR = "/path/software_defects/CWEMAP_data/dataset_name/vpfinder_baseline_results"
...
def stratified_80_10_10_split(rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    try:
        ...
        train_valid_labels = [r["target"] for r in train_valid]
...
def load_dataset_splits() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]], List[str]]:
    ...
    if train_path.exists() and valid_path.exists() and test_path.exists():
        print("Loading existing PrimeVul train/validation/test files.")
...
class VPFinderFunctionDataset(Dataset):
...
class VPFinderBaselineModel(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, labels: Optional[torch.Tensor] = None) -> Dict[str, Any]:
        ...
        # Prefer pooler output if available; otherwise use [CLS].
...
def build_dataloaders(train: List[Dict[str, Any]], valid: List[Dict[str, Any]], test: List[Dict[str, Any]], tokenizer: Any) -> Tuple[DataLoader, DataLoader, DataLoader]:
    train_loader = DataLoader(
        VPFinderFunctionDataset(train, tokenizer),
    ...
    valid_loader = DataLoader(
        VPFinderFunctionDataset(valid, tokenizer),
    ...
    test_loader = DataLoader(
        VPFinderFunctionDataset(test, tokenizer),
        batch_size=EVAL_BATCH_SIZE,
        shuffle=False,
        num_workers=NUM_WORKERS,
        collate_fn=collate_batch,
...
def train_one_epoch(model: VPFinderBaselineModel, loader: DataLoader, optimizer: torch.optim.Optimizer, scheduler: Any, device: torch.device) -> float:
    model.train()
    losses: List[float] = []
...
@torch.no_grad()
def predict(model: VPFinderBaselineModel, loader: DataLoader, device: torch.device, class_names: Sequence[str]) -> List[Dict[str, Any]]:
    model.eval()
    rows: List[Dict[str, Any]] = []
...
def save_outputs(
    ...
    split_summary = pd.DataFrame([
        {"split": "training", "num_examples": len(train), "label_0": label_distribution(train)["0"], "label_1": label_distribution(train)["1"]},
        {"split": "validation", "num_examples": len(valid), "label_0": label_distribution(valid)["0"], "label_1": label_distribution(valid)["1"]},
        {"split": "testing", "num_examples": len(test), "label_0": label_distribution(test)["0"], "label_1": label_distribution(test)["1"]},
    ])
    ...
    xlsx_path = out_dir / "vpfinder_baseline_results.xlsx"
...
def build_optimizer_and_scheduler(model: VPFinderBaselineModel, train_loader: DataLoader) -> Tuple[torch.optim.Optimizer, Any]:
    codebert_params = []
    head_params = []
...
def main() -> None:
    ...
    print("=" * 90)
    print("VPFinder baseline: CodeBERT multi-class CWE classifier over func")
    print("=" * 90)
    ...
    model = VPFinderBaselineModel(CODEBERT_DIR, num_classes=len(class_names), dropout=DROPOUT).to(device)
    optimizer, scheduler = build_optimizer_and_scheduler(model, train_loader)

    best_valid_macro_f1 = -1.0
    bad_epochs = 0
    history: List[Dict[str, Any]] = []
    best_model_path = out_dir / "best_vpfinder_baseline_model.pt"
    ...
    save_outputs(out_dir, train, valid, test, history, valid_preds, test_preds, valid_metrics, test_metrics, config)
    ...
    print(f"Excel     : {out_dir / 'vpfinder_baseline_results.xlsx'}")
